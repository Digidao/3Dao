...
interface IPool{
    ...
    function fundStakeContract(address stakeContract) external returns(uint _standardStakeReward);
}
...
contract StakeContract is GovernanceMath {
    //This contract is called when ProposalPool.sol is 66%
    //The STAKING_PERIOD is proportional to getpoolSupply(in ProposalPool.sol) = 100 or 
    address thisContract;
    address consul;
    uint totalStaked;
    bool isContractCurrent;
    uint public STAKING_PERIOD = 30 days; 
    uint public maxContactValue = 6_800_000e18;
    address public reps;
    address public tokenContract;
    address public governanceContract;
    address public poolContract;
    uint public stakeBonusFunding;
    uint public stakeEndBlock;

    mapping(address => uint) balances;
    mapping(address => uint) stakebonus;
    ...
    modifier onlyMaxContractValue(uint _amount){
        require(_amount < safeSub(maxContactValue,balances[address(0)]), "Proposal pool overflow");
        _;
    }
    ...
    constructor(
        ...
        stakeEndBlock = safeAdd(block.number,STAKING_PERIOD);
        // releaseStake(_oldStakePool);
    }

    function addStake(uint _amount) public onlyReps(msg.sender)onlyMaxContractValue(_amount) returns (string memory message) {
        require(_amount > 0, "Can't stake a 0 value");
        require(isContractCurrent, "Staking period has ended");
        stakeBonusFunding = IPool(poolContract).fundStakeContract(thisContract);
        IToken(tokenContract).transferFrom(msg.sender, poolContract, _amount);
        stakebonus[msg.sender] = div(mul(balances[msg.sender],STAKING_PERIOD), safeSub(block.number,STAKING_PERIOD));
        //START HERE!!!
        balances[msg.sender] = safeAdd(balances[msg.sender], _amount);
        balances[address(0)] = safeAdd(balances[address(0)], _amount);

        totalStaked = safeAdd(totalStaked, _amount);
        message = "You have staked!";
        return message;
    }

    function withdrawStake() public {
        require(!isContractCurrent, "Staking period has not ended");
        ...
        IToken(tokenContract).transfer(msg.sender, stakedAmount);

        // Clear the staked amount for the sender
    ...
    function transfer(address receiver, uint tokens) public returns (bool success) {
        require(isContractCurrent == false, "Staking period has not ended");
    ...
    function transferFrom(address sender, address receiver, uint tokens) public returns (bool success) {
        require(isContractCurrent == false, "Staking period has not ended");
        require(balances[sender] >= tokens, "Insufficient balance");
        require(allowed[sender][msg.sender] >= tokens, "Allowance exceeded");
    ...
    // Additional functions to set the staking period status

    function endStakingPeriod() public {
        require(msg.sender == poolContract, "Only the pool contract can end the staking period");
        isContractCurrent = false;
    }

    function isStakingPeriod() public view returns (bool) {
        return isContractCurrent && block.timestamp < STAKING_PERIOD;
    }
}
